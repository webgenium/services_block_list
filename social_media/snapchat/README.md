# Snapchat — Block List

Pasta de bloqueio para **Snapchat**, contendo domínios, IPs, URLs e ASNs para uso em firewalls.

## Resumo da coleta

| Arquivo | Entradas | Descrição |
|---|---|---|
| `domains.txt` | 24 | Domínios e subdomínios conhecidos |
| `ips.txt` | 8 | Blocos de IP / CIDRs |
| `urls.txt` | 0 | URLs específicas para bloqueio |
| `asn.txt` | 1 | Números de Sistema Autônomo (ASN) |

**Data da coleta:** 2026-03-09

## ASNs

| ASN |
|---|
| `AS395291` |

## Uso no firewall

| Arquivo | Camada | Aplicação |
|---|---|---|
| `domains.txt` | DNS / Layer 7 | Bloqueio por domínio via DNS ou proxy |
| `ips.txt` | Layer 3/4 | Bloqueio por endereço IP ou CIDR |
| `urls.txt` | Layer 7 | Bloqueio por URL específica via proxy |
| `asn.txt` | Layer 3 | Bloqueio de todo o bloco de roteamento do AS |

## Observações

Os dados foram coletados das páginas da Netify.ai e do repositório HaGeZi DNS-Blocklists no GitHub. A lista de IPs da Netify.ai indica "and 34 more" para a seção "Network Ranges" na página de aplicações, o que significa que a lista de IPs pode não ser exaustiva. A lista de URLs específicas para bloqueio não foi encontrada, sendo os domínios listados considerados suficientes para bloqueio em firewall.

---

> **Fonte:** Dados coletados de fontes públicas (bgp.he.net, ipinfo.io, HaGeZi DNS Blocklists, StevenBlack Hosts, documentação oficial).
> Atualizado em: 2026-03-09
